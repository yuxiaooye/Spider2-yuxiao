    ...
    for entry in data:
        ...
        if FLAG:
            ...
            if isinstance(entry['db_id'], str):  
                ...
                for db in entry['db_id']:
                    if db not in available_dbs:
                        print(f"warning: for '{entry['instance_id']}', its required db_id '{db}' is not exist in the metadata. please make sure the metadata is complete!")
